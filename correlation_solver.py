...
def ng_nlx(chi, matrix_sc, matrix_fd, df, field, freq):
    ...
    if pp.verboseError:
        ...
        error = 0
    g_next_nl = -g_next*nl/n # Adjust the g_next to account for the population fraction
    g_next_nx = -g_next*nx/n # Adjust the g_next to account for the population fraction
    print('GMRES convergence criteria: {:3E}'.format(criteria))
    loopend = time.time()
    print('Convergence took {:.2f}s'.format(loopend - loopstart))
    if not pp.simpleBool:
...
def nl_nx(chi, matrix_sc, matrix_fd, df, field, freq):
    ...
    if pp.verboseError:
        ...
        error = 0
    g_next_nl = -g_next*nx/n # Adjust the g_next to account for the population fraction
    print('GMRES convergence criteria: {:3E}'.format(criteria))
    loopend = time.time()
    print('Convergence took {:.2f}s'.format(loopend - loopstart))
    if not pp.simpleBool:
...
def write_ngl_nlx(fieldVector,df,freq):
    ...
    for i in range(len(fieldVector)):
        ...
        g_next_nl, g_next_nx, _, _, _= ng_nlx(chi, scm, fdm, df, EField, freq)
        g_next, _,_, _ = nl_nx(chi, scm, fdm, df, EField, freq)
        del fdm
